...
class EventClose(models.Model):
    ...
    event_type_id = fields.Many2one('event.type', string='Event Category', readonly=True,
    ...
    moderator_id = fields.Many2one('res.users', string='Moderator', readonly=True, related='event_id.user_id')
    designation_id = fields.Many2one('res.users', string="Moderator's Designation")
    company_id = fields.Many2one('res.company', string="Moderator's Organization", readonly=True,
                                 related='moderator_id.company_id')
    theme_id = fields.Many2one('res.users', string='Theme')
    audience_id = fields.Many2one('res.users', string='Key Audience')
    mobile_phone = fields.Char(string="Moderator's Contract Number", readonly=True,
                               related='moderator_id.partner_id.mobile')
    work_email = fields.Char(string="Moderator's Email", readonly=True, related='moderator_id.partner_id.email')
