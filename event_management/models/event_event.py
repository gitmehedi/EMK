...
class EventEvent(models.Model):
    ...
    organizer_id = fields.Many2one('res.partner', string='PoC Name', domain=[('is_poc', '=', True)],
                                   default=False, required=True, readonly=True, states={'draft': [('readonly', False)]})
    ...
    event_type_id = fields.Many2one(string="Event Type", required=True, readonly=True, states={'draft': [('readonly', False)]})
    ...
    user_id = fields.Many2one(string="Organizer Name", required=True, readonly=True, states={'draft': [('readonly', False)]})
    ...
    refundable_amount = fields.Float(string='Refundable Amount', digits=(12, 2), readonly=True,
                                     states={'done': [('readonly', False)]})
    rules_regulation = fields.Html(string='Rules and Regulation',readonly=True, states={'draft': [('readonly', False)]})
    description = fields.Html(readonly=True, states={'draft': [('readonly', False)]})
    date_of_payment = fields.Date(string="Expected Date for Payment", readonly=True, states={'draft': [('readonly', False)]})
    notes = fields.Html(string="Comments/Notes", readonly=True, states={'draft': [('readonly', False)]})
    ref_reservation = fields.Char(string="Reservation Reference", readonly=True, states={'draft': [('readonly', False)]})
    image_medium = fields.Binary(string='Medium-sized photo', attachment=True,readonly=True, states={'draft': [('readonly', False)]})
    participating_amount = fields.Integer(string="Participation Amount", readonly=True, states={'draft': [('readonly', False)]})
    expected_session = fields.Integer(string="No. of Sessions", readonly=True, states={'draft': [('readonly', False)]})
    ...
    target_audience_group = fields.Selection([('yes', 'Yes'), ('no', 'No')], default='yes', required=True,
                                             string="Target Audience Group", readonly=True, states={'draft': [('readonly', False)]})
    target_age = fields.Integer(string="Target Age", required=True, readonly=True, states={'draft': [('readonly', False)]})
    outreach_plan = fields.Selection([('social_media', 'Social Media Promotions'),
                                      ('press_coverage', 'Press Coverage'),
                                      ('designing', 'Designing'),
                                      ('others', 'Others'),
                                      ], string="Outreach Plan", required=True, readonly=True, states={'draft': [('readonly', False)]})
    outreach_plan_other = fields.Char(string="Outreach Plan Other", readonly=True, states={'draft': [('readonly', False)]})
    snacks_required = fields.Selection([('yes', 'Yes'), ('no', 'No')], default='yes',
                                       string="Food/Beverage/Snacks?", readonly=True, states={'draft': [('readonly', False)]})
    ...
    seats_availability = fields.Selection([('limited', 'Limited'), ('unlimited', 'Unlimited')], required=True,
                                          string='Available Seats', default="limited",
                                          readonly=True, states={'draft': [('readonly', False)]})
