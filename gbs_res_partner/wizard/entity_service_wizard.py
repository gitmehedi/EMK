...
class EntityServiceWizard(models.TransientModel):
    _name = 'entity.service.wizard'
    ...
    name = fields.Char(string='Requested Name', default=default_name)

    @api.constrains('name')
    def _check_unique_constrain(self):
        if self.name:
            name = self.env['entity.service'].search(
    ...
    def act_change_name(self):
        ...
        name = self.env['entity.service'].search([('name', '=ilike', self.name)])
        if len(name) > 0:
            raise Warning('[Unique Error] Name must be unique!')

        pending = self.env['history.entity.service'].search([('state', '=', 'pending'), ('line_id', '=', id)])
        if len(pending) > 0:
            raise Warning('[Warning] You already have a pending request!')

        self.env['history.entity.service'].create(
            {'change_name': self.name, 'status': self.status, 'request_date': fields.Datetime.now(), 'line_id': id})
        record = self.env['entity.service'].search(
