odoo.define('gbs_account_reports.account_report_generic', function (require) {
    "use strict";
    ...
    var account_report_generic = core.action_registry.get ('account_report_generic');
    ...
    account_report_generic.include ({
        ...
        get_html: function() {
            ...
            return new Model('account.report.context.common').call('return_context', [self.report_model, self.given_context, self.report_id]).then(function (result) {
                ...
                return self.context_model.call('get_html_and_data', [self.context_id, self.given_context], {context: session.user_context}).then(function (result) {
                    ...
                    self.report_context.available_operating_units = result.report_context.available_operating_units;
        ...
        render_searchview_buttons: function() {
            ...
            this.$searchview_buttons.find('.o_account_reports_one-operating-unit').bind('click', function (event) {
                var report_context = {};
                var value = $(event.target).parents('li').data('value');
                if(self.report_context.operating_unit_ids.indexOf(value) === -1){
                    report_context.add_operating_unit_ids = value;
                }
                else {
                    report_context.remove_operating_unit_ids = value;
                }
                self.restart(report_context);
            });
