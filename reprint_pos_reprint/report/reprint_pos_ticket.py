...
class ReprintSalePosTicket(models.AbstractModel):
    _name = 'report.reprint_pos_report.reprint_pos_ticket'

    @api.multi
    def render_html(self, docids, data=None):
        report_obj = self.env['report']
        report = report_obj._get_report_from_name('reprint_pos_report.reprint_sale_ticket')
        pos_order = self.env['pos.order'].search([('id', '=', docids)])
        ...
        return self.env['report'].render('reprint_pos_report.reprint_pos_ticket', docargs)
