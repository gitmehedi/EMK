...
class BranchIssue(models.Model):
    """
    ...
    def do_transfer(self):
        res =  super(BranchIssue,self).action_done()

        rec_ins = self.search([('origin', '=', self.name)])
        if rec_ins:
            rec_ins.force_assign()
    ...
    #     return super(BranchIssue, self).create(vals)
    #
    # @api.one
    ...
    #         rec.unlink()
    #     return super(BranchIssue, self).unlink()
    #
...
class StockTransferRequestLine(models.Model):
    ...
    avail_qty = fields.Integer(string="Available Quantity")
    requisition_qty = fields.Integer(string="Requisition Quantity")
    issue_qty = fields.Integer(string="Issue Quantity", store=True)
    ...
    #                                                     ('location_id', '=', location_id[0].stock_location_id.id)])
    #
    #         self.avail_qty = sum([val.qty for val in quant])
