...
class MerchandisingDept(models.Model):

	_name = 'merchandising.dept'
	
	name = fields.Char(string='Name', size=30, required=True)
	buyer_ids = fields.Many2many(comodel_name='res.partner',
                          relation='merchandising_dept_relation',
                          column1='merchandising_dept_id',
                          column2='partner_id',domain=[('customer', '=', True)])
	...
	def create(self, vals):
		...
		return super(MerchandisingDept, self).create(vals)
	...
	def write(self, vals):
		...
		if name_value:
			vals['name'] = name_value.strip()
		return super(MerchandisingDept, self).write(vals)
