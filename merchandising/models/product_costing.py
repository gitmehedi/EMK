...
class ProductCosting(models.Model):
    ...
    """
    yarn_ids = fields.One2many('product.costing.line', 'yarn_costing_id', string="Yarn", ondelete="cascade",
                                readonly=True, states={'draft':[('readonly', False)]}, copy=True)
    accessories_ids = fields.One2many('product.costing.accessories', 'accessories_costing_id', string="Accessories",
                                       ondelete="cascade", readonly=True, states={'draft':[('readonly', False)]}, copy=True)
    cm_ids = fields.One2many('cm.costing.line', 'cm_costing_id', string="CM", ondelete="cascade",
                                readonly=True, states={'draft':[('readonly', False)]}, copy=True)
...
#         self._check_sum_percentage(vals)
    ...
    def unlink(self, cr, uid, ids, context=None):
        obj = self.pool['product.costing']

        for item in self.browse(cr, uid, ids, context=context):
            if item.state != 'draft':
                raise exceptions.ValidationError(validator.msg['confirm_delete'])

            # obj.unlink(cr, uid, [line.id for line in item.yarn_ids], context=context)
            # obj.unlink(cr, uid, [line.id for line in item.accessories_ids], context=context)
            # obj.unlink(cr, uid, [line.id for line in item.cm_ids], context=context)
            # obj.unlink(cr, uid, [line.id for line in item.costing_ids], context=context)

        return super(ProductCosting, self).unlink(cr, uid, ids, context=context)
