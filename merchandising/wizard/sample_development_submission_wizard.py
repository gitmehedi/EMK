...
class SampleDevelopmentSubmissionWizard(models.TransientModel):
	_name = 'sample.development.submission.wizard'
	...
	sample_dev_sub_qty = fields.Float(digits=(5, 2), string='Quantity',
	...
	def action_submit(self, cr, uid, ids, context=None):
		...
		dev_obj = self.pool.get('sample.development.submission')
		...
		for data in self.browse(cr, uid, ids):
		    vals = {
		            'sample_dev_sub_date': data.sample_dev_sub_date,
 		         	'sample_requisition_id':req_id,
 		         	'sample_dev_sub_qty':data.sample_dev_sub_qty,
		          	'uom':parent_uom.id
		            }
