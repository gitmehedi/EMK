...
class SampleDevelopmentRequestWizard(models.TransientModel):
	_name = 'sample.development.request.wizard'
...
# 									ondelete='set null', required=True)
	sample_dev_req_qty = fields.Float(digits=(5, 2), string='Quantity',
	...
	def action_submit(self, cr, uid, ids, context=None):
		...
		dev_obj = self.pool.get('sample.development.request')
		...
		for data in self.browse(cr, uid, ids):
		    vals = {
		            'sample_dev_req_date': data.sample_dev_req_date,
		            'sample_requisition_id':req_id,
 		         	'sample_dev_req_qty':data.sample_dev_req_qty,
		          	'uom':parent_uom.id,
		            }
		res = dev_obj.create(cr, uid, vals, context=context)

		req_obj.write(cr, uid, [req_id], {'state':'development'}, context=context)
