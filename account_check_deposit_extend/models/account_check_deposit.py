...
class AccountCheckEntry(models.Model):
    ...
    _description = "Account Check Entry"
    ...
    check_payment_ids = fields.One2many(
        'account.move.line', 'check_entry_id', string='Check Payments',
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].\
                next_by_code('account.check.entry')
        return super(AccountCheckEntry, self).create(vals)
