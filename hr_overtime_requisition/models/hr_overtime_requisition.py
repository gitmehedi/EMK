...
class HROTRequisition(models.Model):
    ...
    def _compute_employee_check_user(self):
        ...
        if user.has_group('base.group_user'):
            if user.has_group('hr.group_hr_manager') or user.has_group('gbs_base_package.group_dept_manager'):
                self.user_access=0
            else:
                self.user_access=1
        else:
    ...
    # user access fields
    user_access_hr_manager = fields.Boolean(string='User Access Hr Manager', compute='_compute_employee_check_hr_manager',readonly=True)
