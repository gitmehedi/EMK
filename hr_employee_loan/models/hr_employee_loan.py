...
class HrEmployeeLoanRequest(models.Model):
    ...
    interst_mode_id = fields.Selection([('flat', 'Flat')], string='Interest Mode',
                                       states={'draft': [('invisible', False)],
                                               'applied': [('readonly', True)],
                                               'approved': [('readonly', True)],
                                               'disbursed': [('readonly', True)]})
    ...
    def onchange_loan_type_id(self):
        res = {}
        self.employee_loan_policy_ids = []
        ids = self.loan_type_id.loan_policy_ids.ids if self.loan_type_id else []
        res['domain'] = {
            'employee_loan_policy_ids': [('id', 'in', ids)],
        }
        return res
