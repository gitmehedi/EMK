...
class FirstAcceptanceInfoXLSX(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        # WORKSHEET
        sheet = workbook.add_worksheet('First Acceptance Report')
        ...
        sheet.merge_range(4, 0, 4, last_col, "First Acceptance Report", name_format)
        ...
        for product in data['product_temp_name']:
            if product:
                ...
                sheet.merge_range(row, 0, row, 3, "Product Name: " + new_product_name, bold)

        sheet.merge_range(6, 12, 6, 14, "Reported On: " + ReportUtility.get_date_time_from_string(current_date_time), bold)
        row += 1
        ...
        sheet.write(row, col + 6, 'Dispatch To Sales', th_cell_center)
        sheet.write(row, col + 7, 'Dispatch To Customer', th_cell_center)
        ...
        sheet.write(row, col + 11, 'Remarks/Status', th_cell_center)
        sheet.write(row, col + 12, 'F.P.R', th_cell_center)
        sheet.write(row, col + 13, 'Bank', th_cell_center)
        sheet.write(row, col + 14, 'Branch', th_cell_center)
        row += 1
        ...
        for product_data in result_data['acceptances']:
            ...
            for rec in result_data['acceptances'][product_data]:
                if rec:
                    ...
                    sheet.write(row, col + 1, rec['customer'], td_cell_center)
                    ...
                    sheet.write(row, col + 6, ReportUtility.get_date_from_string(rec['dispatch_to_sale']), td_cell_center)
                    sheet.write(row, col + 7, ReportUtility.get_date_from_string(rec['dispatch_to_customer']), td_cell_center)
                    sheet.write(row, col + 8, rec['aging'], td_cell_center)
                    sheet.write(row, col + 9, ReportUtility.get_date_time_from_string(rec['first_delivery_date']), td_cell_center)
                    sheet.write(row, col + 10, ReportUtility.get_date_time_from_string(rec['last_delivery_date']), td_cell_center)
                    sheet.write(row, col + 11, rec['comment'], td_cell_center)
                    sheet.write(row, col + 12, rec['sale_persons'], td_cell_center)
                    sheet.write(row, col + 13, rec['bank_code'], td_cell_center)
                    sheet.write(row, col + 14, rec['bank_branch'], td_cell_center)
...
FirstAcceptanceInfoXLSX('report.lc_sales_local_report.lc_first_acceptance_report_xlsx', 'local.first.acceptance.wizard', parser=report_sxw.rml_parse)