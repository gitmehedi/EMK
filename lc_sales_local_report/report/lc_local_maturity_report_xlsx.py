...
class LocalMaturityInfoXLSX(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        result_data = self.env['report.lc_sales_local_report.lc_sales_maturity_temp'].get_report_data(data)
        ...
        # WORKSHEET
        sheet = workbook.add_worksheet('Maturity Report')
        ...
        sheet.merge_range(4, 0, 4, last_col, "Maturity Report", name_format)
        ...
        sheet.merge_range(6, 7, 6, 9, "Reported On: " + ReportUtility.get_date_time_from_string(current_date_time), bold)
        row += 1
        ...
        sheet.write(row, col + 6, 'Customer Bank', th_cell_center)
        ...
        for product_data in result_data['data_list']:
            for rec in result_data['data_list'][product_data]:
                if len(rec)>0:
            ...
            for rec in result_data['data_list'][product_data]:
                if rec:
                    ...
                    sheet.write(row, col + 6, rec['customer_bank'], td_cell_center)
                    sheet.write(row, col + 7, ReportUtility.get_date_from_string(rec['to_maturity_date']), td_cell_center)
                    sheet.write(row, col + 8, rec['aging'], td_cell_center)
                    sheet.write(row, col + 9, rec['bill_id'], td_cell_center)

                    row += 1

            #SUB TOTAL
            for rec in result_data['total'][product_data]:
                for recd in result_data['data_list'][product_data]:
                    if len(recd) > 0:
...
LocalMaturityInfoXLSX('report.lc_sales_local_report.lc_local_maturity_report_xlsx', 'lc.sales.maturity.wizard', parser=report_sxw.rml_parse)