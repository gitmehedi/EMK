...
class CustomerPortal(website_account):
    ...
    def portal_my_appointment(self, appointment_id=None, access_token=None, **kw):
        ...
        appointment = request.env['s2u.appointment.registration'].browse([appointment_id])
        try:
            appointment.check_access_rights('read')
            appointment.check_access_rule('read')
        except AccessError:
            return request.render("website.403")

        appointment_sudo = appointment.sudo()
