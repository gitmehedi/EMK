...
class OnlineAppointment(http.Controller):
    ...
    def prepare_values(self, form_data=False, default_appointee_id=False, criteria='default'):
        ...
        values = {
            ...
            'appointment_date': '',
            'timeslot_id': 0,
            'mode': 'public' if request.env.user == request.website.user_id else 'registered',
            'name': request.env.user.partner_id.name if request.env.user != request.website.user_id else '',
            'email': request.env.user.partner_id.email if request.env.user != request.website.user_id else '',
            'phone': request.env.user.partner_id.phone if request.env.user != request.website.user_id else '',
            'remarks': '',
            'error': {},
            'error_message': [],
    ...
    def online_appointment(self, **kw):
        ...
        if request.env.user == request.website.user_id:
            param = request.env['ir.config_parameter'].sudo().search([('key', '=', 'online_appointment')], limit=1)
            if not param or param.value.lower() != 'public':
                return request.render('online_appointment.only_registered_users', {
                    'signup_enabled': request.env['ir.config_parameter'].sudo().get_param('auth_signup.allow_uninvited') == 'True',
