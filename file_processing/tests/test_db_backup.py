...
model = 'odoo.addons.file_processing.models.db_backup'
...
class TestDbBackup(common.TransactionCase):
    ...
    def new_record(self, method='sftp'):
        ...
        if method == 'sftp':
            vals.update({
                'source_sftp_host': 'test_host',
                'source_sftp_port': '222',
                'source_sftp_user': 'tuser',
                'source_sftp_password': 'password',
                'folder': '/folder/',
            })
        self.vals = vals
    ...
    def test_compute_name_sftp(self):
        ...
        self.assertEqual(
            'sftp://%(user)s@%(host)s:%(port)s%(folder)s' % {
                'user': self.vals['source_sftp_user'],
                'host': self.vals['source_sftp_host'],
                'port': self.vals['source_sftp_port'],
    ...
    def test_sftp_connection_init_passwd(self, pysftp):
        ...
        pysftp.Connection.assert_called_once_with(
            host=rec_id.source_sftp_host,
            username=rec_id.source_sftp_user,
            port=rec_id.source_sftp_port,
            password=rec_id.source_sftp_password,
        )
    ...
    def test_sftp_connection_init_key(self, pysftp):
        ...
        rec_id = self.new_record()
        rec_id.write({
            'source_sftp_private_key': 'pkey',
            'source_sftp_password': 'pkeypass',
        })
        rec_id.sftp_connection()
        pysftp.Connection.assert_called_once_with(
            host=rec_id.source_sftp_host,
            username=rec_id.source_sftp_user,
            port=rec_id.source_sftp_port,
            private_key=rec_id.source_sftp_private_key,
            private_key_pass=rec_id.source_sftp_password,
        )
