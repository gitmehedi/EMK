...
class AccountFiscalyearCloseState(models.TransientModel):
    ...
    _columns = {
       'close_fiscal_year': fields.many2one('account.fiscalyear', \
    ...
    def data_save(self, cr, uid, ids, context=None):
        ...
        for data in  self.read(cr, uid, ids, context=context):
            close_fiscal_year = data['close_fiscal_year'][0]

            account_move_ids = account_move_obj.search(cr, uid, [('period_id.fiscalyear_id', '=', close_fiscal_year), ('state', '=', "draft")], context=context)
            if account_move_ids:
            ...
            cr.execute('UPDATE account_journal_period ' \
                    ...
                    ('done', close_fiscal_year))
            cr.execute('UPDATE account_period SET state = %s ' \
                    'WHERE fiscalyear_id = %s', ('done', close_fiscal_year))
            cr.execute('UPDATE account_fiscalyear ' \
                    'SET state = %s WHERE id = %s', ('done', close_fiscal_year))
