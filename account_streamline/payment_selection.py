...
class good_to_pay(osv.TransientModel):
    ...
    def __compute_sum_and_nb_lines(self, cr, uid, context):
        res = {}
        line_ids = list(itertools.chain.from_iterable(
            context['lines_by_partner'].values()
        ))
        aml_osv = self.pool.get('account.move.line')
        reads = aml_osv.read(cr, uid, line_ids, ['credit', 'debit'], context=context)
        reads = reads if type(reads) == list else [reads]

        total_credit = sum([read['credit'] for read in reads])
        total_debit = sum([read['debit'] for read in reads])
