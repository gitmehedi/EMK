...
class account_move_line(osv.osv):
    ...
    def _get_date_maturity_yday(self, cr, uid, ids, field_name, arg, context):
        move_line_osv = self.pool.get("account.move.line")
        result = {}
        move_lines = move_line_osv.browse(cr, uid, ids, context=context)
        for move_line in move_lines:
            result[move_line.id] = move_line.date_maturity
    ...
    def _is_in_voucher(self, cr, uid, ids, field_name, arg, context=None):
        voucher_line_osv = self.pool['account.voucher.line']
        res = {}
        for _id in ids:
            vline_ids = voucher_line_osv.search(
                cr, uid, [
                    ('move_line_id', '=', _id),
                    ('voucher_id.state', 'in', (
                        'draft', 'proforma', 'validated',
                    )),
                ], context=context
            )
            res[_id] = bool(vline_ids)
        return res

    def _filter_voucher(self, cr, uid, ids, context=None):
        vouchers = self.browse(cr, uid, ids, context=context)
        line_ids = []
        for voucher in vouchers:
            line_ids.extend([move.id for move in voucher.move_ids])
        return line_ids
    ...
    _columns = dict(
        ...
        is_not_payment_batchable=fields.boolean(
            string="Is Not Payment Batchable",
        ),
        is_in_voucher=fields.function(
            _is_in_voucher,
            method=True,
            string="Is in voucher",
            type='boolean',
            store={
                'account.voucher': (
                    _filter_voucher,
        ...
        payment_batch_id=fields.many2one(
            'account.payment_batch',
            string="Payment Batch",
        ),
        date_maturity_yday=fields.function(
            _get_date_maturity_yday,
            method=True,
            string="Year and Year Day",
            type='char',
            store={
                'account.move.line': (
                    lambda self, cr, uid, ids, c={}: ids,
