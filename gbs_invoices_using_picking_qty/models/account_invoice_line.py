            ...
            if rec.invoice_id.type == 'in_invoice':
                if rec.invoice_id.from_po_form:
                    if self.env.user.has_group(
                            'gbs_invoices_using_picking_qty.group_vendor_invoice_editor'):
                        rec.can_edit_bill_line = True
                else:
                    if len(rec.purchase_id) <= 0 or self.env.user.has_group('gbs_invoices_using_picking_qty.group_vendor_invoice_editor'):
                        rec.can_edit_bill_line = True
            # if rec.invoice_id.type == 'in_invoice':
            #     rec.can_edit_bill_line = True
