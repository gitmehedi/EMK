...
openerp.web_widget_distribution_matrix = function(instance)
{
	var instance = openerp;
	var _t = instance.web._t;
	
    instance.web.form.widgets.add(
        'x2many_2d_distribution_matrix',
        'instance.web_widget_distribution_matrix.FieldX2Many2dDistributionMatrix');
    instance.web_widget_distribution_matrix.FieldX2Many2dDistributionMatrix = instance.web.form.FieldOne2Many.extend({
        template: 'FieldX2Many2dDistributionMatrix',
        ...
        get_state_value:function(y)
        {
        	//readonly
        	var state=_.first(_.values(this.by_y_axis[y]))['state']
        	if (state=='confirm') 
        	 return true;
        	else
        		return false;
     ...
     // get the on hand qty for a value on the y axis
        get_y_on_hand_qty_value: function(y)
        {
        	return _.first(_.values(this.by_y_axis[y]))['on_hand_qty'];
        },
        ...
        xy_value_change: function(e)
        {
            ...
            if(this.validate_xy_value(val))
            {
                ...
                $this.closest('tr').children("td.td_onhandqty").children("span").removeClass('oe_form_invalid');
                this.compute_totals();
                var onHandQty=parseFloat($this.closest('tr').children("td.td_onhandqty").children("span").children("input").val().trim());
                var row_total=parseFloat($this.closest('tr').children("td.row_total").text().trim());
                
                if (row_total>onHandQty)
                	{
                		$this.closest('tr').children("td.td_onhandqty").children("span").addClass('oe_form_invalid');
                		this.do_warn(_t("Distribution Error!!!"), "Distribute quantity is grater than on hand quantity")
                	}
            }
            else
            {
