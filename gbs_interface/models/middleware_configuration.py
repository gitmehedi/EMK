...
class ServerFileProcess(models.Model):
    _name = "middleware.configuration"
    ...
    type = fields.Selection([('interface', 'Interface')], default="interface", string='Type', required=True)
    method = fields.Selection(selection=[("sftp", "Double Middleware Location")],
    ...
    def glif_process(self):
        ...
        integration = self.search([('type', '=', 'interface'), ('method', '=', 'sftp'), ('status', '=', True)], limit=1)
        if not integration:
    ...
    # @api.multi
    # def get_paths(self,file):
    #     source_path = os.path.join(self.source_path, file)
    #     dest_path = os.path.join(self.dest_path, file)
    #     local_path = os.path.join(self.folder, file)
    #     with self.sftp_connection('source') as source:
    #         source.get(source_path, localpath=local_path, preserve_mtime=True)
    #
    #                 for file in files:
    ...
    def mdc_generate_process(self):
        ...
        record = self.env['middleware.configuration'].search([('method', '=', 'dest_sftp'),
                                                              ('type', '=', 'mdc'),
                                                              ('status', '=', True)], limit=1)
        for rec in record:
