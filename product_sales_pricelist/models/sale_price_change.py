...
class SalePriceChange(models.Model):
    ...
    product_id = fields.Many2one('product.product', domain=[('sale_ok', '=', True)], states={'confirm': [('readonly', True)], 'validate1': [('readonly', True)], 'validate': [('readonly', True)]}, string='Product', required=True)
    ...
    product_package_mode = fields.Many2one('product.packaging.mode',string='Packaging Mode', states={'confirm': [('readonly', True)], 'validate1': [('readonly', True)], 'validate': [('readonly', True)]},required=True)
    uom_id = fields.Many2one('product.uom', string="UoM", domain=[('category_id', '=', 2)], states={'confirm': [('readonly', True)], 'validate1': [('readonly', True)], 'validate': [('readonly', True)]}, required=True)
    category_id = fields.Many2one(string='UoM Category',related="uom_id.category_id",store=True)
    request_date = fields.Date(string='Request Date',default=datetime.datetime.now(), states={'confirm': [('readonly', True)], 'validate1': [('readonly', True)], 'validate': [('readonly', True)]},readonly=True)
    ...
    approver1_date = fields.Date(string='First Approval Date',states={'confirm': [('readonly', True)], 'validate1': [('readonly', True)], 'validate': [('readonly', True)]}, readonly=True)
    approver2_date = fields.Date(string='Second Approval Date', states={'confirm': [('readonly', True)], 'validate1': [('readonly', True)], 'validate': [('readonly', True)]},readonly=True)
    effective_date = fields.Date(string='Effective Date', states={'confirm': [('readonly', True)], 'validate1': [('readonly', True)], 'validate': [('readonly', True)]}, required=True)
    ...
    def price_change(self):
        if self.product_id:
            ...
            if product_pool:
                ...
                count = self.search_count([('product_id', '=', self.product_id.id),('currency_id', '=', self.currency_id.id),('state', '=', 'validate')])

                if count > 0:
                    price_change_pool = self.env['product.sales.pricelist'].search([('product_id','=',self.product_id.id),
                         ('currency_id','=',self.currency_id.id),('state','=','validate')],order='id desc')[0]
    ...
    def _onchange_currency_id(self):
        if self.product_id:
            ...
            if self.currency_id.name == "BDT":
                count = self.search_count([('product_id', '=', self.product_id.id),('currency_id', '=', self.currency_id.id),
                                           ('state', '=', 'validate')])
                if count > 0:
                    price_change_pool = self.env['product.sales.pricelist'].search(
                        [('product_id','=',self.product_id.id),
    ...
    def action_approve(self):
        ...
        #product_variants_price_history_view.xml
