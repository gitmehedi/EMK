...
class third_party_ledger(report_sxw.rml_parse, common_report_header):
    ...
    def lines(self, partner):
        ...
        self.cr.execute(
            "SELECT max(l.id) as id, max(l.date) as date, max(j.code) as code, max(acc.code) as a_code, max(acc.name) as a_name, " \
            "max(l.ref) as ref, m.name as move_name, max(m.narration) as narration, max(l.name) as name, " \
            "CASE WHEN sum(l.debit)>sum(l.credit) THEN (sum(l.debit)-sum(l.credit)) ELSE 0 END as debit, " \
            "CASE WHEN sum(l.credit)>sum(l.debit) THEN (sum(l.credit)-sum(l.debit)) ELSE 0 END as credit, " \
            "max(l.amount_currency) as amount_currency, max(l.currency_id) as currency_id, max(c.symbol) AS currency_code " \
            ...
            "WHERE l.partner_id = %s " \
                ...
                "AND m.state IN %s " \
                " " + RECONCILE_TAG + " "\
                "GROUP BY move_name ORDER BY date",
                (partner.id, tuple(self.account_ids), tuple(move_state)))
        
        res = self.cr.dictfetchall()
        sum = 0.0
        if self.initial_balance:
            initial_record = self._get_intial_balance(partner)
            if initial_record and initial_record[0][2] <> 0.0:
                val = {'code': '', 'name': '', 'a_code': '', 
                       'move_name': 'Opening Balance', 'currency_id': None,
                       'debit': initial_record[0][0] or 0.0, 
                       'credit': initial_record[0][1] or 0.0, 
                       'a_name': '', 'date': '',
                       'amount_currency': 0.0, 'ref': '', 'id': None, 
                       'currency_code': None, 'progress':initial_record[0][2]}
                sum = initial_record[0][2] or 0.0
                full_account.append(val)
        for r in res:
