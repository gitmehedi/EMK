...
class SaleUnderOverApprovedPriceReportXLSX(ReportXlsx):
    ...
    def _get_sale_products(self, obj):
        ...
        sql_str = """select sol.name as prod_var_name, rp.name as cus_name,so.name as so_no,COALESCE(sol.product_uom_qty, 0) as sold_qty,COALESCE(sol.price_unit_actual,0) as approved_price, COALESCE(sol.price_unit,0) as sale_price,rc.name as currency from sale_order_line as sol 
                    ...
                    """+where_clause+"""
                    group by sol.name, so.date_order, rp.name, so.name, sol.product_uom_qty, sol.price_unit_actual, sol.price_unit, rc.name
