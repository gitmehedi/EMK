...
class EmployeeExitReq(models.Model):
    ...
    reason_for_exit = fields.Text(string='Reason for Leaving', required=True)
    ...
    approver1_by = fields.Many2one('res.users', string="Approved By Department Manager", readonly=True)
    approver2_by = fields.Many2one('res.users', string="Approved By HR Manager", readonly=True)
    confirm_date = fields.Date(string="Confirm Date", default=datetime.date.today(), readonly=True)
    approved1_date = fields.Date(string='Approval Date(Department Manager', readonly=True)
    approved2_date = fields.Date(string="Approval Date(HR Manager)", _defaults=lambda *a: time.strftime('%Y-%m-%d'), readonly=True)
    ...
    #
    #     if vals.get('state') and vals['state'] not in ['draft', 'confirm', 'validate', 'validate1', 'validate2',
    #                                                    'cancel'] and not self.env[
