...
class CurrencyWizard(models.TransientModel):
    ...
    def _check_unique_constrain(self):
        if self.name:
            name = self.env['res.currency'].search(
    ...
    def act_change_name(self):
        ...
        name = self.env['res.currency'].search([('name', '=ilike', self.name)])
        if len(name) > 0:
            raise Warning('[Unique Error] Name must be unique!')

        pending = self.env['history.res.currency'].search([('state', '=', 'pending'), ('line_id', '=', id)])
        if len(pending) > 0:
            raise Warning('[Warning] You already have a pending request!')

        self.env['history.res.currency'].create(
            {'change_name': self.name, 'status': self.status, 'request_date': fields.Datetime.now(), 'line_id': id})
        record = self.env['res.currency'].search(
