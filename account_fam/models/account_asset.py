...
class AccountAssetAsset(models.Model):
    ...
    cost_centre_id = fields.Many2one('account.analytic.account', string='Cost Centre', required=False,
    ...
    def init(self):
        self._cr.execute("""
            CREATE OR REPLACE FUNCTION asset_depreciation(date DATE,user_id INTEGER,journal_id INTEGER,opu_id INTEGER,company_id INTEGER, narr_date TEXT,sys_date DATE) 
                    ...
                    INSERT INTO account_move (name,ref,journal_id,company_id,date,operating_unit_id,state,create_uid,write_uid,create_date,write_date) 
                        VALUES ('/','Asset Depreciation on '|| depr_date ,journal_id,company_id,sys_date,opu_id,'draft',user_id,user_id,sys_date,sys_date)
                    ...
                    LOOP
                          ...
                          INSERT INTO account_move_line (name,ref,journal_id,move_id,account_id,operating_unit_id,analytic_account_id,date_maturity,date,debit,credit,create_uid,write_uid,create_date,write_date,company_id)
                          VALUES ('Depreciation on ' || narr_date,mrec.account_depreciation_id,journal_id,move,mrec.account_depreciation_expense_id,mrec.current_branch_id,mrec.cost_centre_id,sys_date,sys_date,mrec.depr_sum,0,user_id,user_id,NOW(),NOW(),company_id);
                          -- insert debit amount in account.move.line
                          INSERT INTO account_move_line (name,ref,journal_id,move_id,account_id,operating_unit_id,analytic_account_id,date_maturity,date,debit,credit,create_uid,write_uid,create_date,write_date,company_id)
                          VALUES ('Accu. Depr. on ' || narr_date,mrec.account_depreciation_id,journal_id,move,mrec.account_depreciation_id,mrec.current_branch_id,mrec.cost_centre_id,sys_date,sys_date,0,mrec.depr_sum,user_id,user_id,NOW(),NOW(),company_id);
    ...
    def create_move(self, line, date):
        ...
        if line:
            ...
            move_line_1 = {
                'name': 'Depreciation on ' + category_id.name + date.strftime(' - %b, %Y'),
            ...
            move_line_2 = {
                'name': 'Accu. Depr. on ' + date.strftime(' - %b, %Y'),
