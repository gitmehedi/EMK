...
class AccountAssetSell(models.Model):
    ...
    total_sell_amount = fields.Float(string='Total Sell Value', compute='_compute_total_sell_amount')
    ...
    sell_date = fields.Datetime(string='Dispose Date', readonly=True, states={'approve': [('readonly', False)]})
    ...
    sell_user_id = fields.Many2one('res.users', string='Dispose User', readonly=True,
                                   states={'approve': [('readonly', False)]}, )
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.user.company_id.id)
    note = fields.Text(string='Note', readonly=True, states={'draft': [('readonly', False)]}, )

    line_ids = fields.One2many('account.asset.sell.line', 'sell_id', string='Sell Line', readonly=True,
                               states={'draft': [('readonly', False)]}, )
    state = fields.Selection([('draft', 'Draft'), ('approve', 'Approved'), ('sell', 'Sold')], default='draft',
                             string='State')
    ...
    def _compute_total_sell_amount(self):
        for rec in self:
            rec.total_sell_amount = sum(rec.sell_value for rec in rec.line_ids)

    @api.one
    def action_approve(self):
        if len(self.line_ids) <= 0:
            raise ValidationError(_("Sell List should not empty."))

        if self.state == 'draft':
            self.state = 'approve'
            self.approve_date = fields.Datetime.now()
            self.approve_user_id = self.env.user.id
            self.name = self.env['ir.sequence'].next_by_code('account.asset.sell') or _('New')
    ...
    def action_sell(self):
        if self.state == 'approve':
            ...
            self.state = 'sell'
            self.sell_date = fields.Datetime.now()
            self.sell_user_id = self.env.user.id

    @api.multi
    def unlink(self):
        for rec in self:
            if rec.state in ('approve', 'sell'):
...
class AccountAssetSellLine(models.Model):
    ...
    sell_value = fields.Float(string='Sell Value', required=True, digits=(14, 2))

    sell_id = fields.Many2one('account.asset.sell', string='Sell', ondelete='restrict')
