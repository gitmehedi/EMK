...
class StockInventoryWizard(models.TransientModel):
    ...
    def print_report(self):
        ...
        location = self.env['stock.location'].search([('operating_unit_id', '=', res.shop_id.id)])
        location_outsource = location.id

        ctx['form'] = self.read([], ['date_from', 'date_to'])[0]
        cat_pool = self.env['product.category']
        if self.category_id:
            categories = cat_pool.get_categories(self.category_id.id)
        else:
            categories = cat_pool.search([]).ids

        if len(categories) == 1:
            category_param = "(" + str(self.category_id.id) + ")"
        else:
