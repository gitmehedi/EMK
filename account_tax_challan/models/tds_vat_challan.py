...
class TdsVatChallan(models.Model):
    ...
    challan_no = fields.Char(string='Challan No.', track_visibility='onchange', readonly=True, required=True,
                             states={'draft': [('readonly', False)]})
    deposited_bank = fields.Char(string='Deposited Bank', track_visibility='onchange', readonly=True, required=True,
                                 states={'draft': [('readonly', False)]})
    bank_branch = fields.Char(string='Bank Branch', track_visibility='onchange', readonly=True, required=True,
                              states={'draft': [('readonly', False)]})
    ...
    def action_print(self):
        return self.env['report'].get_action(self, 'account_tax_challan.report_tds_vat_challan')
    ...
    def create(self, vals):
        if vals['acc_move_line_ids']:
            move_objs = self.env['account.move.line'].search(
                [('id', 'in', [i[2] for i in vals['acc_move_line_ids']][0])])
